-

# Atelier PHP – Authentification : Comprendre la Sécurité Web en Pratique

## 🎯 Objectif Pédagogique





## 🧠 Le Paradigme Mental : Le Château Fort Numérique

**Imaginez votre site web comme un château fort :**

- **HTTP Basic** → Le garde à l'entrée qui demande une carte
- **Formulaire + Session** → Le laissez-passer temporaire  
- **Hashage** → Le coffre-fort pour les secrets
- **CSRF** → Le sceau royal anti-contrefaçon
- **JWT** → Le passeport vérifiable

---

## 🛡️ 1. HTTP Basic : Le Garde Simple mais Efficace

### Scénario : Accès à l'API d'administration

```php
<?php
// 🎯 ANALOGIE : Le garde qui demande "Qui va là ?" à chaque fois
error_reporting(E_ALL);
ini_set('display_errors', 1);

echo "<h3>🔐 Porte d'Entrée HTTP Basic</h3>";

// 🎯 LE GARDE RÉCUPÈRE VOTRE CARTE
$user = $_SERVER['PHP_AUTH_USER'] ?? null;    // Votre nom
$pass = $_SERVER['PHP_AUTH_PW'] ?? null;      // Votre mot de passe

// 🎯 LA LISTE DES AUTORISÉS
$AUTORISES = [
    'admin' => 'secret123',
    'technicien' => 'tech2024'
];

// 🎯 VÉRIFICATION DE LA CARTE
if (!$user || !$pass || !isset($AUTORISES[$user]) || $AUTORISES[$user] !== $pass) {

    // 🚨 REFUS D'ACCÈS - Le garde vous bloque
    header('WWW-Authenticate: Basic realm="Zone Admin - Identifiez-vous"');
    header('HTTP/1.0 401 Unauthorized');

    echo "<div style='background:#f8d7da; color:#721c24; padding:20px;'>";
    echo "🚫 <strong>Accès refusé</strong> - Vous n'êtes pas autorisé à entrer.";
    echo "</div>";
    exit;
}

// 🎯 ACCÈS AUTORISÉ - Le garde vous laisse passer
echo "<div style='background:#d1edff; color:#004085; padding:20px;'>";
echo "✅ <strong>Bienvenue $user !</strong> Accès autorisé à la zone admin.";
echo "</div>";

// 🎯 CONTENU PROTÉGÉ
echo "<div style='margin-top:20px; padding:15px; background:#f8f9fa;'>";
echo "<h4>📊 Tableau de Bord Administratif</h4>";
echo "<ul>";
echo "<li>📈 Statistiques du site</li>";
echo "<li>👥 Gestion des utilisateurs</li>";  
echo "<li>⚙️ Paramètres système</li>";
echo "</ul>";
echo "</div>";
?>
```

**🧪 Testez :** Allez sur cette page → Une popup apparaît → Essayez admin/secret123

**💡 Le POURQUOI :**

- **Simple** : Pas besoin de formulaire HTML
- **Universel** : Tous les navigateurs comprennent
- **Usage** : APIs, outils d'admin, accès rapide

**⚠️ Limites :** Mot de passe en clair dans la requête → Toujours utiliser HTTPS !

---

## 📝 2. Formulaire + Sessions : Le Laissez-Passer Temporaire

### Scénario : Application web avec connexion utilisateur

```php
<?php
// 🎯 ANALOGIE : Le bureau d'accueil qui délivre un badge visiteur
session_start();

// 🎯 SI DÉJÀ CONNECTÉ → REDIRECTION
if (isset($_SESSION['user'])) {
    header('Location: dashboard.php');
    exit;
}

// 🎯 GESTION DES ERREURS
$error = '';
if (isset($_SESSION['login_error'])) {
    $error = $_SESSION['login_error'];
    unset($_SESSION['login_error']);
}
?>
<!DOCTYPE html>
<html>
<head>
    <title>🏢 Connexion à l'Espace Membre</title>
    <style>
        .login-box { max-width: 400px; margin: 50px auto; padding: 30px; background: white; border-radius: 10px; box-shadow: 0 0 20px rgba(0,0,0,0.1); }
        .form-group { margin: 20px 0; }
        .error { background: #f8d7da; color: #721c24; padding: 10px; border-radius: 5px; }
    </style>
</head>
<body style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; display: flex; align-items: center;">
    <div class="login-box">
        <h2 style="text-align: center; color: #333;">🔐 Connexion</h2>

        <?php if ($error): ?>
            <div class="error">❌ <?= htmlspecialchars($error) ?></div>
        <?php endif; ?>

        <form action="process_login.php" method="post">
            <div class="form-group">
                <label><strong>📧 Email :</strong></label>
                <input type="email" name="email" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
            </div>

            <div class="form-group">
                <label><strong>🔒 Mot de passe :</strong></label>
                <input type="password" name="password" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 5px;">
            </div>

            <button type="submit" style="width: 100%; padding: 12px; background: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">
                🚀 Se connecter
            </button>
        </form>

        <!-- 🧠 ZONE PÉDAGOGIQUE -->
        <div style="margin-top: 20px; padding: 15px; background: #f8f9fa; border-radius: 5px;">
            <h4>💡 Comment ça marche ?</h4>
            <p><strong>Session PHP = Badge temporaire</strong></p>
            <p>Le serveur vous donne un identifiant unique qui prouve que vous êtes connecté.</p>
            <p><strong>Compte de test :</strong> user@example.com / pass123</p>
        </div>
    </div>
</body>
</html>
```

**Processus de connexion :**

```php
<?php
// process_login.php
session_start();

// 🎯 RÉCUPÉRATION ET NETTOYAGE DES DONNÉES
$email = trim($_POST['email'] ?? '');
$password = $_POST['password'] ?? '';

// 🎯 BASE D'UTILISATEURS (SIMULATION)
$utilisateurs_valides = [
    'user@example.com' => 'pass123', // En réalité, on utilisera du hashage !
    'admin@site.com' => 'admin123'
];

// 🎯 VÉRIFICATION DES IDENTIFIANTS
if (isset($utilisateurs_valides[$email]) && $utilisateurs_valides[$email] === $password) {

    // ✅ CONNEXION RÉUSSIE - Création du badge session
    $_SESSION['user'] = [
        'email' => $email,
        'nom' => 'Utilisateur Test',
        'login_time' => date('Y-m-d H:i:s')
    ];

    // 🎯 REGÉNÉRATION DE L'ID DE SESSION (sécurité)
    session_regenerate_id(true);

    header('Location: dashboard.php');
    exit;
} else {
    // ❌ ÉCHEC - Message d'erreur
    $_SESSION['login_error'] = "Email ou mot de passe incorrect";
    header('Location: login.php');
    exit;
}
?>
```

**Tableau de bord protégé :**

```php
<?php
// dashboard.php
session_start();

// 🎯 VÉRIFICATION DU BADGE - Si pas connecté → retour accueil
if (!isset($_SESSION['user'])) {
    header('Location: login.php');
    exit;
}

$user = $_SESSION['user'];
?>
<!DOCTYPE html>
<html>
<head>
    <title>📊 Tableau de Bord</title>
</head>
<body style="font-family: Arial, sans-serif;">
    <div style="background: #343a40; color: white; padding: 20px;">
        <h1>🏢 Espace Membre</h1>
        <p>Connecté en tant que : <strong><?= htmlspecialchars($user['email']) ?></strong></p>
    </div>

    <div style="padding: 20px;">
        <h2>📈 Vos Statistiques</h2>

        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin: 20px 0;">
            <div style="background: #e3f2fd; padding: 20px; border-radius: 8px;">
                <h3>📊 Profil</h3>
                <p>Complétude : 75%</p>
            </div>
            <div style="background: #e8f5e8; padding: 20px; border-radius: 8px;">
                <h3>🕒 Session</h3>
                <p>Connecté depuis : <?= $user['login_time'] ?></p>
            </div>
        </div>

        <a href="logout.php" style="display: inline-block; padding: 10px 20px; background: #dc3545; color: white; text-decoration: none; border-radius: 5px;">
            🚪 Se déconnecter
        </a>
    </div>
</body>
</html>
```

---

## 🔐 3. Hashage Sécurisé : Le Coffre-Fort des Mots de Passe

### Scénario : Comprendre pourquoi on ne stocke JAMAIS les mots de passe en clair

```php
<?php
// 🎯 ANALOGIE : Transformer votre mot de passe en empreinte digitale unique
error_reporting(E_ALL);
ini_set('display_errors', 1);

echo "<h2>🔐 Le Coffre-Fort des Mots de Passe</h2>";

$mot_de_passe_original = "MonSuperSecret123";

// 🎯 CRÉATION DE L'EMPREINTE SÉCURISÉE
$hash_securise = password_hash($mot_de_passe_original, PASSWORD_DEFAULT);

echo "<div style='background:#e8f5e8; padding:15px; margin:10px 0;'>";
echo "<h3>✅ Empreinte créée :</h3>";
echo "<code style='background:#2c3e50; color:white; padding:10px; display:block;'>$hash_securise</code>";
echo "</div>";

// 🎯 VÉRIFICATION DE L'EMPREINTE
$tentative_connexion = "MonSuperSecret123";
$verification = password_verify($tentative_connexion, $hash_securise);

echo "<div style='background:" . ($verification ? '#d4edda' : '#f8d7da') . "; padding:15px;'>";
echo "<h3>" . ($verification ? "✅ Accès autorisé" : "❌ Accès refusé") . "</h3>";
echo "<p>Tentative avec : <strong>$tentative_connexion</strong></p>";
echo "<p>Résultat : " . ($verification ? "L'empreinte correspond !" : "L'empreinte ne correspond pas") . "</p>";
echo "</div>";

// 🎯 DÉMONSTRATION DE LA PUISSANCE DU HASHAGE
echo "<div style='background:#fff3cd; padding:15px; margin-top:20px;'>";
echo "<h3>💡 Pourquoi c'est si sécurisé ?</h3>";
echo "<ul>";
echo "<li><strong>Irréversible :</strong> Impossible de retrouver le mot de passe depuis l'empreinte</li>";
echo "<li><strong>Unique :</strong> Le même mot de passe donne une empreinte différente à chaque fois</li>";
echo "<li><strong>Lent :</strong> Calcul intentionnellement lent pour résister aux attaques</li>";
echo "</ul>";

// 🎯 DÉMONSTRATION DE L'UNICITÉ
$hash2 = password_hash($mot_de_passe_original, PASSWORD_DEFAULT);
echo "<p><strong>Même mot de passe, empreinte différente :</strong></p>";
echo "<code style='background:#2c3e50; color:white; padding:5px; font-size:12px;'>$hash2</code>";
echo "</div>";
?>
```

**Intégration dans le processus de connexion :**

```php
<?php
// 🎯 INSCRIPTION - Création du compte
$mot_de_passe_utilisateur = "MonNouveauMotDePasse";
$hash_a_stocker = password_hash($mot_de_passe_utilisateur, PASSWORD_DEFAULT);

// 🎯 Stocker $hash_a_stocker dans la base de données
// JAMAIS le mot de passe en clair !

// 🎯 CONNEXION - Vérification
$mot_de_passe_saisi = $_POST['password'];
$hash_de_la_base = "..." // Récupéré depuis la base de données

if (password_verify($mot_de_passe_saisi, $hash_de_la_base)) {
    // ✅ Mot de passe correct !

    // 🎯 MISE À JOUR SI NÉCESSAIRE (algorithme amélioré)
    if (password_needs_rehash($hash_de_la_base, PASSWORD_DEFAULT)) {
        $nouveau_hash = password_hash($mot_de_passe_saisi, PASSWORD_DEFAULT);
        // Mettre à jour dans la base de données
    }
} else {
    // ❌ Mot de passe incorrect
}
?>
```

---

## 🛡️ 4. Protection CSRF : Le Sceau Anti-Contrefaçon

### Scénario : Empêcher les attaques où on vous fait cliquer sur des liens malveillants

```php
<?php
// 🎯 ANALOGIE : Le sceau royal qui authentifie les documents officiels
session_start();

// 🎯 CRÉATION DU SCEAU UNIQUE
if (empty($_SESSION['csrf_token'])) {
    $_SESSION['csrf_token'] = bin2hex(random_bytes(32)); // 64 caractères hexadécimaux
}

$token_csrf = $_SESSION['csrf_token'];
?>
<!DOCTYPE html>
<html>
<head>
    <title>🏦 Virement Bancaire Sécurisé</title>
    <style>
        .transaction-box { max-width: 500px; margin: 30px auto; padding: 20px; background: white; border-radius: 10px; box-shadow: 0 0 15px rgba(0,0,0,0.1); }
        .security-badge { background: #d4edda; color: #155724; padding: 10px; border-radius: 5px; margin: 10px 0; }
    </style>
</head>
<body style="background: #f8f9fa;">
    <div class="transaction-box">
        <h2>💸 Effectuer un Virement</h2>

        <div class="security-badge">
            🔒 <strong>Transaction sécurisée par jeton CSRF</strong>
        </div>

        <form action="process_transaction.php" method="post">
            <div style="margin: 15px 0;">
                <label><strong>👤 Bénéficiaire :</strong></label>
                <input type="text" name="beneficiaire" required style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
            </div>

            <div style="margin: 15px 0;">
                <label><strong>💰 Montant :</strong></label>
                <input type="number" name="montant" min="1" required style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">
            </div>

            <!-- 🎯 LE SCEAU CACHÉ - Invisible mais essentiel -->
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($token_csrf) ?>">

            <button type="submit" style="width: 100%; padding: 12px; background: #28a745; color: white; border: none; border-radius: 5px; cursor: pointer;">
                ✅ Confirmer le virement
            </button>
        </form>

        <!-- 🧠 EXPLICATION PÉDAGOGIQUE -->
        <div style="margin-top: 20px; padding: 15px; background: #e3f2fd; border-radius: 5px;">
            <h4>🛡️ Comment CSRF vous protège :</h4>
            <p><strong>Scénario d'attaque :</strong> Un site malveillant vous fait cliquer sur un lien qui envoie un virement sans votre consentement.</p>
            <p><strong>Protection CSRF :</strong> Chaque formulaire a un <strong>sceau unique</strong> que seul notre site connaît.</p>
            <p><strong>Résultat :</strong> Les sites malveillants ne peuvent pas forger vos requêtes !</p>
        </div>
    </div>
</body>
</html>
```

**Vérification côté serveur :**

```php
<?php
// process_transaction.php
session_start();

// 🎯 VÉRIFICATION DU SCEAU
if (!isset($_POST['csrf_token']) || !isset($_SESSION['csrf_token'])) {
    http_response_code(403);
    die("🚫 Sceau manquant - Transaction refusée");
}

// 🎯 COMPARAISON SÉCURISÉE DES SCEAUX
if (!hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
    http_response_code(403);
    die("🚫 Sceau invalide - Tentative de contrefaçon détectée");
}

// 🎯 RÉGÉNÉRATION DU SCEAU (usage unique)
$_SESSION['csrf_token'] = bin2hex(random_bytes(32));

// ✅ TRANSACTION AUTORISÉE
$beneficiaire = htmlspecialchars($_POST['beneficiaire']);
$montant = floatval($_POST['montant']);

echo "<div style='background:#d4edda; color:#155724; padding:20px; text-align:center;'>";
echo "<h2>✅ Virement confirmé !</h2>";
echo "<p><strong>$montant €</strong> ont été envoyés à <strong>$beneficiaire</strong></p>";
echo "<p>Le sceau CSRF a protégé cette transaction contre les attaques.</p>";
echo "</div>";
?>
```

---

## 🏆 Mini-Projet Final : Système d'Authentification Complet

### Architecture Professionnelle avec Sécurité Multi-couches

**Structure du projet :**

```
auth_system/
├── 🗂️ config/          # Configuration
│   └── security.php
├── 🗂️ includes/        # Fonctions réutilisables  
│   ├── auth.php
│   ├── csrf.php
│   └── database.php
├── 🗂️ pages/           # Pages de l'application
│   ├── login.php
│   ├── register.php
│   ├── dashboard.php
│   └── profile.php
└── 🗂️ assets/          # CSS, JS, images
    └── style.css
```

**Fichier de configuration :**

```php
<?php
// config/security.php

// 🎯 CONFIGURATION DE SÉCURITÉ
session_start([
    'cookie_httponly' => true,     // Empêche l'accès JS aux cookies
    'cookie_secure' => true,       // Uniquement en HTTPS
    'use_strict_mode' => true      // Sécurité renforcée
]);

// 🎯 CONSTANTES DE SÉCURITÉ
define('MAX_LOGIN_ATTEMPTS', 5);
define('LOGIN_TIMEOUT', 900); // 15 minutes
define('CSRF_TOKEN_LIFETIME', 3600); // 1 heure

// 🎯 FONCTION ANTI BRUTE-FORCE
function check_brute_force($email) {
    $key = 'login_attempts_' . md5($email);

    if (!isset($_SESSION[$key])) {
        $_SESSION[$key] = ['count' => 0, 'first_attempt' => time()];
    }

    $attempts = &$_SESSION[$key];

    // 🎯 RÉINITIALISATION SI TIMEOUT DÉPASSÉ
    if (time() - $attempts['first_attempt'] > LOGIN_TIMEOUT) {
        $attempts = ['count' => 0, 'first_attempt' => time()];
    }

    // 🎯 VÉRIFICATION DU NOMBRE D'ESSAIS
    if (++$attempts['count'] > MAX_LOGIN_ATTEMPTS) {
        $remaining_time = LOGIN_TIMEOUT - (time() - $attempts['first_attempt']);
        throw new Exception("Trop de tentatives. Réessayez dans " . ceil($remaining_time/60) . " minutes.");
    }

    return $attempts['count'];
}
?>
```

**Système d'authentification complet :**

```php
<?php
// includes/auth.php

require_once __DIR__ . '/../config/security.php';

class Authentification {

    // 🎯 INSCRIPTION D'UN NOUVEL UTILISATEUR
    public static function inscrire($email, $password, $nom) {
        // Validation des données
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            throw new Exception("Format d'email invalide");
        }

        if (strlen($password) < 8) {
            throw new Exception("Le mot de passe doit faire au moins 8 caractères");
        }

        // 🎯 HASHAGE SÉCURISÉ
        $hash_password = password_hash($password, PASSWORD_DEFAULT);

        // 🎯 SIMULATION BASE DE DONNÉES
        $utilisateur = [
            'email' => $email,
            'password_hash' => $hash_password,
            'nom' => $nom,
            'date_inscription' => date('Y-m-d H:i:s'),
            'role' => 'utilisateur'
        ];

        // Ici, normalement : $db->insert('users', $utilisateur);
        $_SESSION['users'][$email] = $utilisateur;

        return true;
    }

    // 🎯 CONNEXION
    public static function connecter($email, $password) {
        try {
            // 🎯 VÉRIFICATION ANTI BRUTE-FORCE
            $attempts = check_brute_force($email);

            // 🎯 RÉCUPÉRATION UTILISATEUR
            $utilisateur = $_SESSION['users'][$email] ?? null;

            if (!$utilisateur) {
                throw new Exception("Utilisateur non trouvé");
            }

            // 🎯 VÉRIFICATION DU MOT DE PASSE
            if (!password_verify($password, $utilisateur['password_hash'])) {
                throw new Exception("Mot de passe incorrect");
            }

            // 🎯 MISE À JOUR DU HASH SI NÉCESSAIRE
            if (password_needs_rehash($utilisateur['password_hash'], PASSWORD_DEFAULT)) {
                $utilisateur['password_hash'] = password_hash($password, PASSWORD_DEFAULT);
                $_SESSION['users'][$email] = $utilisateur;
            }

            // 🎯 CRÉATION DE LA SESSION
            $_SESSION['user'] = [
                'email' => $utilisateur['email'],
                'nom' => $utilisateur['nom'],
                'role' => $utilisateur['role'],
                'login_time' => time(),
                'session_id' => session_id()
            ];

            // 🎯 RÉINITIALISATION DES TENTATIVES
            unset($_SESSION['login_attempts_' . md5($email)]);

            // 🎯 REGÉNÉRATION DE L'ID DE SESSION
            session_regenerate_id(true);

            return true;

        } catch (Exception $e) {
            throw $e;
        }
    }

    // 🎯 VÉRIFICATION DE CONNEXION
    public static function est_connecte() {
        return isset($_SESSION['user']);
    }

    // 🎯 DÉCONNEXION
    public static function deconnecter() {
        $_SESSION = [];

        if (ini_get("session.use_cookies")) {
            $params = session_get_cookie_params();
            setcookie(session_name(), '', time() - 42000,
                $params["path"], $params["domain"],
                $params["secure"], $params["httponly"]
            );
        }

        session_destroy();
    }

    // 🎯 RÉCUPÉRATION DE L'UTILISATEUR COURANT
    public static function utilisateur_courant() {
        return $_SESSION['user'] ?? null;
    }
}
?>
```

**Page de connexion finale :**

```php
<?php
// pages/login.php
require_once '../includes/auth.php';
require_once '../includes/csrf.php';

// 🎯 REDIRECTION SI DÉJÀ CONNECTÉ
if (Authentification::est_connecte()) {
    header('Location: dashboard.php');
    exit;
}

$erreur = $_SESSION['login_error'] ?? '';
unset($_SESSION['login_error']);
?>
<!DOCTYPE html>
<html>
<head>
    <title>🔐 Connexion Sécurisée</title>
    <link rel="stylesheet" href="../assets/style.css">
</head>
<body class="auth-page">
    <div class="auth-container">
        <div class="auth-card">
            <h1 class="auth-title">🔐 Connexion</h1>

            <?php if ($erreur): ?>
                <div class="alert alert-danger">
                    <strong>❌ Erreur :</strong> <?= htmlspecialchars($erreur) ?>
                </div>
            <?php endif; ?>

            <form action="../handlers/login_handler.php" method="post" class="auth-form">
                <div class="form-group">
                    <label>📧 Adresse email</label>
                    <input type="email" name="email" required class="form-control" 
                           placeholder="votre@email.com" autocomplete="email">
                </div>

                <div class="form-group">
                    <label>🔒 Mot de passe</label>
                    <input type="password" name="password" required class="form-control" 
                           placeholder="Votre mot de passe" autocomplete="current-password">
                </div>

                <!-- 🎯 PROTECTION CSRF -->
                <?= CSRF::champ() ?>

                <button type="submit" class="btn btn-primary btn-block">
                    🚀 Se connecter
                </button>
            </form>

            <div class="auth-links">
                <a href="register.php">📝 Créer un compte</a>
                <a href="forgot_password.php">🔓 Mot de passe oublié ?</a>
            </div>

            <!-- 🛡️ INDICATEURS DE SÉCURITÉ -->
            <div class="security-indicators">
                <div class="indicator">
                    <span class="icon">🔒</span>
                    <span>HTTPS Actif</span>
                </div>
                <div class="indicator">
                    <span class="icon">🛡️</span>
                    <span>Protection CSRF</span>
                </div>
                <div class="indicator">
                    <span class="icon">⚡</span>
                    <span>Hash Bcrypt</span>
                </div>
            </div>
        </div>
    </div>
</body>
</html>
```

---

## 🎯 Checklist de Sécurité Finale

**Vous avez maintenant implémenté :**

- ✅ **Authentification HTTP Basic** pour les APIs
- ✅ **Sessions sécurisées** avec régénération d'ID
- ✅ **Hashage bcrypt** pour les mots de passe
- ✅ **Protection CSRF** avec tokens uniques
- ✅ **Limitation de tentatives** anti brute-force
- ✅ **Validation des entrées** utilisateur
- ✅ **Cookies sécurisés** (httpOnly, secure)
- ✅ **Architecture modulaire** professionnelle

**Vous êtes passé de "développeur" à "architecte de sécurité" !** 🎉
